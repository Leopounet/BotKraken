...
class AssetPairData:
    ...
    def __str__(self : AssetPairData) -> str:
        """
        """
        s = f"Current:                    {round(self.current, 6)}\n"
        s += f"Maximum:                    {round(self.maximum, 6)}\n"
        s += f"Minimum:                    {round(self.minimum, 6)}\n"
        s += f"Variance:                   {round(self.variance, 6)}\n"
        s += f"Mean:                       {round(self.mean, 6)}\n"
        s += f"Longest ascension:          {round(self.longest_ascension, 6)}\n"
        s += f"Shortest ascension:         {round(self.shortest_ascension, 6)}\n"
        s += f"Average ascension:          {round(self.average_ascension, 6)}\n"
        s += f"Is ascending:               {self.is_ascending}\n"
        s += f"Current ascension duration: {round(self.current_ascension_duration, 6)}\n"
        s += f"Trending upwards:           {self.trending_upwards}\n"
        s += f"Longest fall:               {round(self.longest_fall, 6)}\n"
        s += f"Shortest fall:              {round(self.shortest_fall, 6)}\n"
        s += f"Average fall:               {round(self.average_fall, 6)}\n"
        s += f"Is falling:                 {self.is_falling}\n"
        s += f"Current fall duration:      {round(self.current_fall_duration, 6)}\n"
        s += f"Trending downwards:         {self.trending_downwards}\n"
        s += f"Average local maximum:      {round(self.average_local_max, 6)}\n"
        s += f"Number of local maximums:   {round(self.nb_local_max, 6)}\n"
        s += f"Average local minimum:      {round(self.average_local_min, 6)}\n"
        s += f"Number of local minimums:   {round(self.nb_local_min, 6)}"
        return s